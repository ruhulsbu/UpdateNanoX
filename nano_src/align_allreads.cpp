...
void align_reads(vector<pair<string, string> >& reference, string& read_file, string& sam_file, vector<reference_index>& refindex)
{
	...
	*/
	...
	while(!fp_read.eof())
	{
		...
		if(read_name.find("unaligned") != std::string::npos)
			continue;
				...
				vector<pair<int, vector<pair<int, int> > > >& primary_chain)
{
	...
	unordered_map<long, int> kmer_last_index;
	...
	for(int k = 0; k < read.length(); k++)
	{
	 	...
	 	*/
	 	...
	 	if(kmer_last_index.find(hash_key) == kmer_last_index.end())
		{
                	kmer_list.push_back(make_pair(hash_key, k - KMER + 1));
			kmer_last_index[hash_key] = kmer_list.size() - 1;
			/*
			prehashval = hash_key;
			prehashcnt = 1;
			*/
		...
		else
		{
			...
			*/
			...
			if((k - KMER + 1) - kmer_list[kmer_last_index[hash_key]].second < MINREPEATDIST)
			{
				kmer_list[kmer_last_index[hash_key]].second = k - KMER + 1;
			}
			else
			{
				kmer_list.push_back(make_pair(hash_key, k - KMER + 1));
                                ...
                                string read, int dir, vector<reference_index>& refindex, int index)
{
	...
	vector<pair<int, int> > long_chain;
	int last_start, last_end, overlap_index;

	last_start = max(0, head->ref_ind - head->read_ind);
	last_end = min(refindex[index].ref.length(), head->ref_ind + (read.length() - head->read_ind));

	while(current_node != NULL)
	{
		//cout << "\nStarting Chain Creation: " << endl;
		//cout << "Ref = " << current_node->ref_ind << ", And Read = " << current_node->read_ind << endl;
		//cout << "Last Start = " << last_start << ", Last End = " << last_end << endl;
		//if(current_node->read_ind < readlen / 2)
		
		overlap_index = max(0, current_node->ref_ind - current_node->read_ind);
		//cout << "Overlap Index = " << overlap_index << endl;
		if(overlap_index >= last_start && overlap_index <= last_end)
		{
			//vector<pair<int, int> > chain;
                	long_chain.push_back(make_pair(current_node->ref_ind, current_node->read_ind));
                	create_primary_chain_from_list(current_node, long_chain, current_node->next, current_node, readlen);
			//cout << "Size of Chain is = " << long_chain.size() << endl; 
		...
		{
			if(long_chain.size() > 1)
			{
                		primary_chain.push_back(make_pair(index * MULTIPLIER + dir, long_chain));
				//cout << "Size of Chain is = " << long_chain.size() << endl << endl; 
			}
	
			//cout << "Terminating Chain" << endl << endl;
			long_chain.clear();
			last_start = max(0, current_node->ref_ind - current_node->read_ind);
			last_end = min(refindex[index].ref.length(), current_node->ref_ind + (read.length() - current_node->read_ind));
